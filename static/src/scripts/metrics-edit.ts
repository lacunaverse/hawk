...
const data = document.body.querySelector('div#data') as HTMLDivElement;
const metric = {
    name: data.dataset.name,
    type: data.dataset.type,
    frequency: data.dataset.frequency,
};

(() => {
    Object.entries(metric).forEach(([k, v]) => {
        (document.body.querySelector(`#metric-${k}`) as HTMLInputElement).value = String(v);
    });
})();
...
submit.addEventListener('click', () => {
    ...
    if (isValid == undefined) {
        ...
        fetch(`/metrics/edit/${metric.name}`, {
            body: serialized,
            method: 'POST',
